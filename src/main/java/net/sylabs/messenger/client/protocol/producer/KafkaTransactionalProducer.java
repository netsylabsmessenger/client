...
public class KafkaTransactionalProducer<MessageType extends MessageInterface> implements TransactionalProducerInterface<MessageType> {
    ...
    private final List<String> messagesToCommit;
    ...
    public KafkaTransactionalProducer(
        ...
        this.messagesToCommit = new ArrayList<>();
    }
    ...
    public void produce(List<MessageType> messages) throws ProducerException {
        ...
        for (MessageInterface message : messages) {
            if (!messageClass.equals(message.getClass())) {
                throw new ProducerException(
                    String.format(
                        "First argument must be an array of %s instances.",
        ...
        try {
            ...
            prepareTransaction(serializedMessages);
        } catch (Throwable exception) {
            throw new ProducerException(exception.getMessage(), exception);
        }
    }
    ...
    @Override
    public void commitProduction() throws ProducerException {
        try {
            for (String message : messagesToCommit) {
        ...
        try {
            ...
            try {
                ...
                throw new ProducerException(exception.getMessage(), exception2);
            }
    ...
    private void prepareTransaction(List<String> messages) throws ProducerException {
        ...
        messagesToCommit.addAll(messages);
    }

    private KafkaProducer<String, String> createKafkaProducer() throws ProducerException {
        try {
            ...
            properties.setProperty(org.apache.kafka.clients.producer.ProducerConfig.TRANSACTIONAL_ID_CONFIG, producerId);
